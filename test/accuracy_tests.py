...
from model_evaluation_scripts.classifiers_classes_api.mistral_API_classifier import mistral_large_classifier
from model_evaluation_scripts.classifiers_classes_api.mixtral_8x7b_API_classifier import mistral_classifier

import matplotlib.pyplot as plt
import seaborn as sns
import time
from dotenv import main as mn
...
def plot_toxicity_comparison(human_toxicity_scores, ai_toxicity_scores):
    ...
    sns.scatterplot(x=human_toxicity_scores, y=human_toxicity_scores, label='Human', color='black')
    ...
    plt.xlabel('Moda de la toxicidad de los sujetos')
...
def plot_toxicity_models_scatter_plot(model, model_name, file, columns, save_plot = True, use_mode = False):
    user_labels = []
    model_predictions = []
    start = time.time()
    ...
    for column in columns:
        ...
        for value in file[column]:
            ...
            if pd.notna(value):
                ...
                toxicity_values.append(column_toxicity)

                valid_responses += 1
        
        real_toxicity = toxicity_message_users/valid_responses
        if use_mode: real_toxicity = pd.Series(toxicity_values).mode()[0]
...
def main():
    ...
    mistral_scores, user_scores = plot_toxicity_models_scatter_plot(mistral, "Mixtral 8x7B", df, columnas_18_hasta_final, use_mode=True)
    gpt_scores, user_scores = plot_toxicity_models_scatter_plot(gpt, "GPT 3.5", df, columnas_18_hasta_final, use_mode=True)
    ...
    plot_toxicity_comparison(user_scores, {"Mixtral 8 X 7B": mistral_scores, "GPT 3.5": gpt_scores})
